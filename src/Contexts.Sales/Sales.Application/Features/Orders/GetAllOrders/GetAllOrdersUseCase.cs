...
public class GetAllOrdersUseCase
{
    ...
    public async Task<PaginatedList<OrderDto>> ExecuteAsync(int page, int pageSize, Guid customerId, CancellationToken cancellationToken = default)
    {
        if (page <= 0) page = 1;
        if (pageSize <= 0) pageSize = 10;

        var (items, totalCount) = await _orderRepository.GetAllAsync(page, pageSize, customerId, cancellationToken);
